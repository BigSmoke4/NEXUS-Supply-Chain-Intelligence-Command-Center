...
public class RiskController : Controller
{
    ...
    public RiskController(NexusDbContext db, IRiskScoringService riskScoring, ICurrentTenant tenant)
    {
        _db = db;
    ...
    public async Task<IActionResult> Heatmap(CancellationToken ct)
    {
        ...
        var supplierNodes = await _db.SupplyNodes
            .Where(n => n.OrganizationId == orgId && n.Type == NodeType.Supplier)
            .ToListAsync(ct);
        ...
        var products = await _db.Products.Where(p => p.OrganizationId == orgId).ToDictionaryAsync(p => p.Id, ct);
