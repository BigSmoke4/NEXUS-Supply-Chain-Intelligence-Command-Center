...
public class ScenariosController : Controller
{
    ...
    public ScenariosController(
        ...
        IValidator<RunScenarioRequest> validator, INaturalLanguageScenarioParser nlParser, NexusMetrics metrics)
    {
        _db = db;
        _jobQueue = jobQueue;
    ...
    public async Task<IActionResult> Builder(CancellationToken ct)
    {
        ...
        var nodes = org is null
            ? new List<SupplyNode>()
            : await _db.SupplyNodes.Where(n => n.OrganizationId == org.Id
                && (n.Type == NodeType.Supplier || n.Type == NodeType.Factory || n.Type == NodeType.Warehouse))
    ...
    public async Task<IActionResult> Run(Guid nodeId, DisruptionType type, int durationDays, double severityPercent, string? name, CancellationToken ct)
    {
        ...
        if (!validation.IsValid)
        {
            ...
            var org = await _db.Organizations.FirstAsync(ct);
            ViewBag.Nodes = await _db.SupplyNodes.Where(n => n.OrganizationId == org.Id
                    && (n.Type == NodeType.Supplier || n.Type == NodeType.Factory || n.Type == NodeType.Warehouse))
                .OrderBy(n => n.Name).ToListAsync(ct);
            return View("Builder");
        }

        var org2 = await _db.Organizations.FirstAsync(ct);
        ...
        await _jobQueue.EnqueueAsync(new SimulationJob(org2.Id, scenario.Id), ct);

        await _audit.RecordAsync(
            org2.Id, User?.Identity?.Name, "Scenario.Queued", nameof(Scenario), scenario.Id, ct: ct);
