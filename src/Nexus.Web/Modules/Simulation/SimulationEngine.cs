...
public class SimulationEngine : ISimulationEngine
{
    ...
    public async Task<ScenarioResult> RunAsync(Guid scenarioId, CancellationToken ct = default)
    {
        ...
        var result = new ScenarioResult { ScenarioId = scenario.Id };
        ...
        {
            timeline.Add(new TimelineEvent { DayOffset = 0, Label = $"Disruption begins: {scenario.Disruptions.First().Type}", Severity = "critical" });
        }
        ...
        foreach (var disruptedNodeId in capacityFactor.Keys)
        {
            ...
            {
                cascadeSteps.Add(new CascadeStep { NodeId = disruptedNodeId, NodeName = originNode.Name, NodeType = originNode.Type.ToString(), StageOrder = 0 });
            }
            ...
            foreach (var (n, depth) in downstreamWithDepth)
            {
                ...
                if (cascadeVisited.Add(n.Id))
                    cascadeSteps.Add(new CascadeStep { NodeId = n.Id, NodeName = n.Name, NodeType = n.Type.ToString(), StageOrder = depth });
            }
        }
