...
public static class SeedData
{
    public static async Task SeedAsync(NexusDbContext db)
    {
        ...
        db.BillOfMaterials.AddRange(
            new BillOfMaterial { ProductId = productA.Id, ComponentId = componentX.Id, QuantityPerUnit = 1 },
            new BillOfMaterial { ProductId = productA.Id, ComponentId = componentY.Id, QuantityPerUnit = 2 },
            new BillOfMaterial { ProductId = productB.Id, ComponentId = componentX.Id, QuantityPerUnit = 1 },
            new BillOfMaterial { ProductId = productC.Id, ComponentId = componentX.Id, QuantityPerUnit = 3 },
            new BillOfMaterial { ProductId = productC.Id, ComponentId = componentZ.Id, QuantityPerUnit = 1 }
        );
        ...
        SupplyNode[] existingFactoryNodes, SupplyNode[] existingWarehouseNodes)
    {
        ...
        for (int i = 0; i < additionalProducts; i++)
        {
            ...
            for (int b = 0; b < bomComponentCount; b++)
            {
                var comp = components[rng.Next(components.Count)];
                boms.Add(new BillOfMaterial { ProductId = p.Id, ComponentId = comp.Id, QuantityPerUnit = rng.Next(1, 4) });
            }
