...
public static class SeedData
{
    public static async Task SeedAsync(NexusDbContext db)
    {
        ...
        // Persist the organization before creating tenant-owned records. The
        // DbContext write boundary intentionally rejects tenant-owned writes
        // when no tenant is resolved, so establish the tenant only after the
        // organization itself exists. This also makes startup seeding behave
        // correctly in hosted/integration-test environments with no HttpContext.
        await db.SaveChangesAsync();

        using var tenantScope = Nexus.Web.Security.HttpContextCurrentTenant.UseTenant(org.Id);
